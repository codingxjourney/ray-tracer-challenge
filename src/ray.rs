...
impl Ray {
    pub fn new(origin: Tuple, direction: Tuple) -> Self {
        if !origin.is_point() || !direction.is_vector() {
            panic!("Origin argument should be point and Direction argument should be vector");
        }

        Ray { origin, direction }
    }

    pub fn position(&self, time: F) -> Tuple {
        self.origin + self.direction * time

        //this will not work because I have not implemented multiplying `f64` by `tuple::Tuple`
...
mod tests {
    use super::*;
    use crate::{fuzzy_eq::*, matrix::Matrix};

    #[test]
    fn creating_and_quering_ray() {
